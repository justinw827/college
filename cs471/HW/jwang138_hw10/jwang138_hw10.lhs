...
s2 takes 2 integer parameters and returns an integer.
The value of 10 applied to s1 is 109
...
Source: https://en.wikibooks.org/wiki/Haskell/Lists_III
...
> insertAt v p (x:xs)
>   | p > (length (x:xs) = (x:xs) ++ [v]
>   | otherwise = [x] ++ (insertAt v (p-1) xs)
...
Works with format composeList "7 [ (-) 3 , (*) 2, (+)5 ]" but couldn't get it to work with above example format

> composeList :: Num a => a -> [a -> a] -> a
> composeList a [] = a
> composeList a (x:xs) = x (composeList a xs)

Source: https://stackoverflow.com/questions/4342013/the-composition-of-functions-in-a-list-of-functions
