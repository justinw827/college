...
NOTE: You may match two elements at a time against a list: [X,Y|Xs] = List. Its preferable to do it in the rule head however...
some_rule([X,Y|Xs]) :- ...  */
...
NOTE: Dont worry about the error cases: i.e, N greater than the length of Y.  */
...
zip([],[],[]).
zip([H|T],[H2|T2],[(H,H2)|Z]) :- zip(T,T2,Z).
...
merge([],M,M).
merge(M,[],M).
merge([HA|TA],[HB|TB],[HA|M]) :-
    HA > HB, 
    merge(TA,[HB|TB],M).
merge([HA|TA],[HB|TB],[HB|M]) :- 
    HA =< HB, 
    merge([HA|TA],TB,M).
