...
insert(X, node(A, Y, Z), node(A1, Y, Z)) :- 
      A = A1,
      insert(A, Y, Z),
      insert(A1, Y, Z).
...
to_list(node(_,L,_), Y) :- to_list(L, Y).
to_list(node(X,_,_), [X|Z]).
to_list(node(_,_,R), Y) :- to_list(R, Y).
...
evaluate(Eq, Res, Val) :- Val = A:B, Eq = Res.
...
binaryAP(node(_, T), Y) :- binaryAP(T, Y).
binaryAP(node(X,_), [X|Z]) :- binaryAP(node(_), L). % Add operator to list if next node is a number
binaryAP(node(_,L,_), Y) :- binaryAP(L, Y).
binaryAP(node(X,_,_), [X|Z]) :- binaryAP(node(_), L). % Add operator to list if next node is a number
binaryAP(node(_,_,R), Y) :- binaryAP(R, Y).
...
noAtoms(A, 0) :- number(A). % Check if A is a number

noAtoms(A, 1) :- atom(A). % Check if A is an atom
...
/* Problem 9 Test */
% Youre own your own for this one :) */
...
change(0, []).
change(A, [(C, Num)|L]) :- 
      coin(C, Val), % Get coin value
      A >= Val, 
      Num is floor(A / Val), % floor(+Expr) Evaluate Expr and return the largest integer smaller or equal to the result of the evaluation.
      B is A mod Val, 
      change(B, L).
